...
from .shopper_node import get_cached_kapruka_tools
...
async def logistics_node(state: ShoppingGraphState) -> Dict[str, Any]:
    ...
    # 1. Fetch tools from the globally active Kapruka session
    ...
    if response.tool_calls:
        ...
        for tool_call, tool_msg in zip(response.tool_calls, tool_messages):
            try:
                ...
                elif isinstance(content, str) and content.strip().startswith("[{") and "}]" in content:
                ...
                print(f"Warning: Could not parse JSON from {tool_call['name']}: {str(e)}")
                continue
